...
public class ThemePreviewCommand implements SubCommand {

    private final ThemeRepository themeRepository;

    public ThemePreviewCommand(ThemeRepository themeRepository) {
    ...
    public void execute(MessageReceivedEvent event, String[] args) {
        ...
        event.getGuild().getChannels().forEach(channel -> {
            if (channel.getParent() == null && channel.getType() != ChannelType.CATEGORY) {
        ...
        event.getGuild().getCategories().forEach(cat -> {
            ...
            cat.getChannels().forEach(channel -> {
                sb2.append(" # ").append(getChannelText(theme, channel)).append("\n");
            });
            sb2.append("```");
            builder.addField("", sb2.toString(), false);
        });
